  ...
  return (
    <section id="features-section" className="py-20 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16 bg-black/20 backdrop-blur-sm p-6 rounded-xl border border-white/10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((feature, index) => (
            <Card 
              key={index} 
              className={`border border-white/20 bg-white/10 backdrop-blur-sm hover:bg-white/20 transition-all duration-500 hover:translate-y-[-4px] ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
              style={{
            ...
            >
              <CardContent className="p-6">
                <div className="mb-4 bg-wildfire-900/30 w-14 h-14 rounded-lg flex items-center justify-center">{feature.icon}</div>
